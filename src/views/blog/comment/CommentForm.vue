  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      ...
      <el-form-item label="评论人会员编号" prop="userId">
        <el-input v-model="formData.userId" placeholder="请输入评论人会员编号" />
      </el-form-item>
      <el-form-item label="评论内容" prop="content">
        <Editor v-model="formData.content" height="150px" />
      </el-form-item>
      <el-form-item label="父评论编号，0 表示一级评论" prop="parentId">
        <el-input v-model="formData.parentId" placeholder="请输入父评论编号，0 表示一级评论" />
      </el-form-item>
      <el-form-item label="根评论编号（一级评论为自身 id）" prop="rootId">
        <el-input v-model="formData.rootId" placeholder="请输入根评论编号（一级评论为自身 id）" />
      </el-form-item>
      <el-form-item label="被回复的会员编号" prop="replyUserId">
        <el-input v-model="formData.replyUserId" placeholder="请输入被回复的会员编号" />
      </el-form-item>
      <el-form-item label="是否作者置顶（仅一级评论）" prop="pinned">
        <el-radio-group v-model="formData.pinned">
          <el-radio value="1">请选择字典生成</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item label="置顶时间" prop="pinTime">
        <el-date-picker
          v-model="formData.pinTime"
          type="date"
          value-format="x"
          placeholder="选择置顶时间"
        />
...
const formRules = reactive({
  ...
  userId: [{ required: true, message: '评论人会员编号不能为空', trigger: 'blur' }],
  content: [{ required: true, message: '评论内容不能为空', trigger: 'blur' }],
  parentId: [{ required: true, message: '父评论编号，0 表示一级评论不能为空', trigger: 'blur' }],
  rootId: [{ required: true, message: '根评论编号（一级评论为自身 id）不能为空', trigger: 'blur' }],
  pinned: [{ required: true, message: '是否作者置顶（仅一级评论）不能为空', trigger: 'blur' }]
})
